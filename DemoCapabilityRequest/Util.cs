...
namespace DemoUtilities
{
    ...
    public class Util
    {
        ...
        public static string GetDefaultTSLocation()
        {
#if !(FNE_NETCORE)
            return Environment.GetFolderPath(Environment.SpecialFolder.Personal) + Path.DirectorySeparatorChar;
#else
            string strPath = String.Empty;
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                strPath += Environment.GetEnvironmentVariable("HOMEDRIVE") + Environment.GetEnvironmentVariable("HOMEPATH");
            }
            else
            {
                strPath += Environment.GetEnvironmentVariable("HOME");
            }
            if (Directory.Exists(strPath + Path.DirectorySeparatorChar + "Documents"))
            {
                strPath += Path.DirectorySeparatorChar + "Documents";
            }
            return strPath;
#endif
        }
